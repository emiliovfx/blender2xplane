...
class SCENE_PT_xplane(bpy.types.Panel):
    ...
    def draw(self, context):
        #layout.prop(scene.xplane, "collection_sort_order", expand=True)
        scene = context.scene
...
def scene_layout(layout: bpy.types.UILayout, scene: bpy.types.Scene):
    ...
    row.prop(scene.xplane, "collection_sort_order", expand=True)
    exp_box.separator()

    all_collections = xplane_helpers.get_collections_in_scene(scene)[1:]
    reverse = scene.xplane.collection_sort_order == "DESC"

    root_exportables = sorted(
        (c for c in all_collections if c.xplane.is_exportable_collection),
        key=lambda c: c.name.lower(),
        reverse=reverse,
    )

    for collection in root_exportables:
    ...
    if scene.xplane.expanded_non_exporting_collections:
        if scene.xplane.expanded_non_exporting_collections:
            # FILTER UI — MUST be drawn before using the value
            filter_row = non_exp_box.row(align=True)
            filter_row.prop(
                scene.xplane,
            ...
            flt = (scene.xplane.other_collections_filter or "").strip().lower()

        non_exportables = sorted(
            (
                c for c in all_collections
                if (not c.xplane.is_exportable_collection)
                   and (not flt or flt in c.name.lower())
            ),
            key=lambda c: c.name.lower(),
            reverse=reverse,
        )

        for collection in non_exportables:
...
def animation_layout(
    ...
    # NOTE (Blender 4.4+/4.5/5.0):
    # In Pose Mode, the Bone Properties context still gives you context.bone, but
    # UI keyframe highlighting (yellow/green) depends on the *ID owner* of the RNA path.
    # Bone dataref keyframes are inserted on the ARMATURE DATA datablock:
    #   armature.data.keyframe_insert(data_path='bones["Bone"].xplane.datarefs[i].value', ...)
    # If we draw props from a detached bone pointer, Blender can't always resolve the owner,
    # so the field won't tint yellow/green even though keyframes exist.
    # Force the bone pointer to come from context.object.data.bones[...] so the owner is Armature data.
    if is_bone:
        arm_obj = getattr(bpy.context, "object", None)
        if arm_obj is not None and getattr(arm_obj, "type", None) == "ARMATURE":
            bone_name = getattr(obj, "name", None)
            if bone_name:
                bone_from_data = arm_obj.data.bones.get(bone_name)
                if bone_from_data is not None:
                    obj = bone_from_data
    layout.separator()
    row = layout.row()
