...
from io_xplane2blender.xplane_utils.xplane_anim_compat import get_fcurves_for_anim_data
    ...
    def collectAnimations(self) -> None:
        ...
        try:
            if bone:
                ...
                fcurves = [
                    f
                    for f in get_fcurves_for_anim_data(blenderObject.data.animation_data)
                    if f.data_path.startswith(f'bones["{bone.name}"].xplane.datarefs')
                ]
            else:
                fcurves = [
                    f
                    for f in get_fcurves_for_anim_data(blenderObject.animation_data)
