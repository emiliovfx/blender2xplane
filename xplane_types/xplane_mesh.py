    ...
    def collectXPlaneObjects(self, xplaneObjects: List[XPlaneObject]) -> None:
        ...
        for xplaneObject in xplaneObjects:
            if (
                ...
                # NOTE: Custom/weighted normals are stored in custom data layers.
                # If we don't preserve those layers, Blender can drop custom loop normals
                # (e.g. produced by the Weighted Normal modifier), resulting in wrong shading
                # in the exported OBJ.
                ...
                # Ensure loop (a.k.a. split/corner) normals are available.
                # When custom normals exist, the per-loop normals reflect them, so later we
                # prefer mesh.loops[...].normal over loop_triangle.split_normals.
                ...
                for tri in mesh.loop_triangles:
                    tmp_face = TempFace(
                        ...
                        normal=tri.normal,
                        # Prefer per-loop normals (they preserve custom/weighted normals).
                        # Keep tri.split_normals as a fallback.
                        split_normals=tuple(
                ...
                for tmp_face in tmp_faces:
                    ...
                    for i in reversed(range(0, 3)):
                        ...
                        normal = xplane_helpers.vec_b_to_x(
                            # Prefer loop (corner) normals whenever they are available.
                            # In practice, artists expect custom/weighted normals to export as
                            # smooth shading, even if the evaluated mesh drops "use_smooth" flags.
                            tmp_face.split_normals[i]
                            if (
                                tmp_face.original_face.use_smooth
                                or getattr(mesh, "has_custom_normals", False)
                            )
