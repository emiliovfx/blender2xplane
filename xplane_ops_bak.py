...
from io_xplane2blender.xplane_utils.xplane_anim_compat import get_fcurves_for_anim_data, remove_fcurve_from_collection
...
from io_xplane2blender.xplane_utils.xplane_anim_compat import ensure_action_group
...
def makeKeyframesLinear(obj, path):
    ...
    if (
        ...
        and len(get_fcurves_for_anim_data(obj.animation_data)) > 0
    ):
        fcurve = findFCurveByPath(get_fcurves_for_anim_data(obj.animation_data), path)
...
class OBJECT_OT_remove_xplane_dataref(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if (
            ...
            fcurve = findFCurveByPath(get_fcurves_for_anim_data(obj.animation_data), path)
            if fcurve:
                remove_fcurve_from_collection(get_fcurves_for_anim_data(obj.animation_data), fcurve)
...
class BONE_OT_add_xplane_dataref(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # PoseBone data path (same as Add operator)
        path = f'pose.bones["{bone.name}"].xplane.datarefs[{self.index}].value'

        # Delete keyframe from ARMATURE OBJECT (pose animation lives here)
        armature.keyframe_delete(data_path=path, group=groupName)
...
class BONE_OT_add_xplane_dataref_keyframe(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # Bone animations live on the ARMATURE DATA datablock
        if armature.data.animation_data is None:
            armature.data.animation_data_create()

        ensure_action_group(armature.data.animation_data, groupName)
